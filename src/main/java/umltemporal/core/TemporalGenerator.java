...
public class TemporalGenerator {
    ...
    private UmlNode decision(UmlNode node, Deque<Expression> stack) {
        ...
        // with the same decisionF
        List<UmlNode> leftNextOutgoing = graph.getOutgoing(leftNextNode);
        // this signalizes the end of loop - next(MERGE that is assoc. with decision) = the false branch of loop
        if (leftNextNode == right) { //leftNextOutgoing.size() == 1 && leftNextOutgoing.get(0) == node) {
    ...
    /**
     * Save the fact that we already visited this merge node which is associated
     * with a decision
     */
    private void saveDecisionMergeVisit(UmlNode mergeNode) {
    }
}
